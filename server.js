...
function createComment(url, request) {
  ...
  if (commentBody &&
    database.users[commentUsername] && database.articles[commentArticleId]) {
...
function deleteComment(url, request) {
  ...
  if (savedComment) {
    ...
    const artComms = article && article.commentIds;
    if (artComms) {
      artComms.splice(artComms.indexOf(id), 1);
    }

    const user = database.users[savedComment.username];
    const userComms = user && user.commentIds;
    if (userComms) {
      userComms.splice(userComms.indexOf(id), 1);
    }
...
const yamlDbName = 'scoop-db.yaml';

function loadDatabase() {
  try {
    const yamlDb = yaml.safeLoad(fs.readFileSync(yamlDbName, 'utf8'));
    database = yamlDb;
    console.log('yaml');
  } catch (e) {
    if (e.message.startsWith('ENOENT: no such file')) {
      console.log('(Starting with an empty DB)');
    } else {
